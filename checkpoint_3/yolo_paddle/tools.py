...
def loss(pred_conf, pred_cls, pred_txtytwth, label):
    ...
    cls_loss_function = nn.CrossEntropyLoss(reduction='none',axis=1)
    txty_loss_function = nn.BCEWithLogitsLoss(reduction='none')
    twth_loss_function = nn.MSELoss(reduction='none')

    pred_conf = nn.functional.sigmoid(pred_conf[:, :, 0])
    pred_cls = paddle.transpose(pred_cls,perm=[0, 2, 1])
    pred_txty = pred_txtytwth[:, :, :2]
    pred_twth = pred_txtytwth[:, :, 2:]
        
    gt_obj = label[:, :, 0].astype('float32')
    gt_cls = label[:, :, 1].astype('int64')
    gt_txtytwth = label[:, :, 2:-1].astype('float32')
